/* pm6 .. tarted to split the line into words, by counting how many words,
...
#define GRPSCRDEL 380000 /*  group screen dleay in microseconds */
#define BTWNSCRDEL 300000 /*  between screen delay */

#define TIMPERGRPSZ 0 /*  set to one if we want a stay-on-screen relative to group/paragraph size */

linkty *newlink(char *linstr, uint lsz, uint idx, uint wds) /* takes a string and integer and creates the first link in an ll from them */
{
    linkty *newp;
...
void print_in_middle(WINDOW *win, int midvert, int startx, int width, linkty *string, int nl)
{
    ...
    for(j=0;j<nl;++j) {
        ...
        x = startx + (int)temp;
        mvwprintw(win, vposarr[j], x, "%s...%u", listp->lstr, listp->wn);
        refresh();
