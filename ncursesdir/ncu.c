/* ncurses example swept form the inet 
 *
 * compile with -lncurses at the end of course 
 * 
 */
#include <time.h>
#include <unistd.h>
...
void screen_init(void) {
   ...
   refresh(); // 1)
   wrefresh(mainwnd);
   screen = newwin(13, 67, 1, 1); // arg1 is height, arg2 width
...
static void update_display(void) {
   ...
   mvwprintw(screen,3,6,"TIME: %d:%02d:%d", now_hour, now_min, now_sec);
...
int main(void) {
   screen_init();
   while (doloop) {
      current_getch = getch(); // holds the latest key press
      if (current_getch == 113) { // could be the ascii character cod of q
         doloop = 0;
      }
      maketime();
      update_display();
