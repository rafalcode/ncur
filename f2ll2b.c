...
linkty *flines2ll(char *fname, uint *numels, uint *blankid) /* will render a file's lines into a linked list, will also record numels */
{
    FILE *fp=fopen(fname,"r");

    size_t gbuf = GENBUF, bbuf = GENBUF;
    char *lines=calloc(gbuf, sizeof(char));
    uint *bids=calloc(bbuf, sizeof(uint)); /*  will hand over to blankid afterwards */
